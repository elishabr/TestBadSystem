...
void gammaln(double *x)
{
  ...
  if (*x > 2.55E+305) {
    ...
    *x = -log(*x) + *x * (*x * (xnum / xden) - 0.57721566490153287);
  } else if (*x <= 0.6796875) {
    double xden;
    double xnum;
    xden = 1.0;
    xnum = 0.0;
    for (i = 0; i < 8; i++) {
      xnum = xnum * ((*x - 0.5) - 0.5) + p2[i];
      xden = xden * ((*x - 0.5) - 0.5) + q2[i];
    }
    *x = -log(*x) + ((*x - 0.5) - 0.5) * (((*x - 0.5) - 0.5) * (xnum / xden) +
                                          0.42278433509846713);
    ...
    *x = table100[(int)*x - 1];
    ...
    xnum = 0.0;
    for (i = 0; i < 8; i++) {
      xnum = xnum * ((*x - 0.5) - 0.5) + p1[i];
      xden = xden * ((*x - 0.5) - 0.5) + q1[i];
    }
    *x = ((*x - 0.5) - 0.5) *
         (((*x - 0.5) - 0.5) * (xnum / xden) - 0.57721566490153287);
  } else if (*x <= 4.0) {
    double xden;
    double xnum;
    xden = 1.0;
    xnum = 0.0;
    for (i = 0; i < 8; i++) {
      xnum = xnum * (*x - 2.0) + p2[i];
      xden = xden * (*x - 2.0) + q2[i];
    }
    *x = (*x - 2.0) * ((*x - 2.0) * (xnum / xden) + 0.42278433509846713);
  } else if (*x <= 12.0) {
    double xden;
    double xnum;
    xden = -1.0;
    xnum = 0.0;
    for (i = 0; i < 8; i++) {
      xnum = xnum * (*x - 4.0) + p4[i];
      xden = xden * (*x - 4.0) + q4[i];
    }
    *x = (*x - 4.0) * (xnum / xden) + 1.791759469228055;
  } else {
    double xden;
    double xnum;
    if (*x <= 2.25E+76) {
      xnum = 0.0057083835261;
      xden = 1.0 / (*x * *x);
      for (i = 0; i < 6; i++) {
        xnum = xnum * xden + c[i];
      }
    ...
    xden = log(*x);
    *x = ((xnum + 0.91893853320467278) - 0.5 * xden) + *x * (xden - 1.0);
  }
}
